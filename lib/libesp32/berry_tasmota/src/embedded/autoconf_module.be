...
autoconf_module.init = def (m)
  ...
  class Autoconf
    ...
    def load_templates()
      ...
      import json
      try 
        var url = format("https://raw.githubusercontent.com/sushantr5/EZIE/main/autoconf/%s_manifest.json", tasmota.arch())
    ...
    def page_autoconf_ctl()
      ...
      try
        if webserver.has_arg("reapply")
          ...
          if arch_name != "reset"
            var url = format("https://raw.githubusercontent.com/sushantr5/EZIE/main/autoconf/%s/%s.autoconf", tasmota.arch(), arch_name)
    ...
    def autoexec()
      ...
      if path.exists(fname) && self.is_first_time()
        ...
        try
          ...
          print(format("MTR CFG: could not copy '_matter_device.json' (%s - %s)'", e, m))
          if f != nil   f.close()   end

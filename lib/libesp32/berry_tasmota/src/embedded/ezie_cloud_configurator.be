...
class ezie_cloud_configurator
  ...
  def init(in_owner_email, in_mqtt_username)
    ...
    tasmota.add_cmd('resetmqtt', def() self.resetmqtt() end)
  ...
  def resetmqtt()
    ...
    cl.begin("https://wqxpc1agpf.execute-api.eu-west-1.amazonaws.com/live/claim_ownership")
    var r = cl.POST("")
    print(r)
    if r == 301 || r == 302
  ...
  def show_info()
    ...
    webserver.content_send("<p>First Reset your MQTT Credentials:</p>")
    webserver.content_send("<p></p><button type='submit' name='action' value='resetcredentials'>Reset Your MQTT Credentials</button><p></p>")
    webserver.content_send("<p></p><p>and then reset device MQTT settings:</p>")
    webserver.content_send("<p></p><button type='submit' name='action' value='resetmqtt'>Reset MQTT device settings</button><p></p>")
    webserver.content_send("<p></p></fieldset><p></p>")
  ...
  def show_mqtt_settings()
    ...
    webserver.content_send("<p></p><button type='submit' name='action' value='changemqttcredentials'>Change MQTT Credentials</button><p></p>")
  ...
  #######################################################################
  # Web Controller, called by POST to `/eziecloudconf`
  #######################################################################
  def page_part_ctl()
    import webserver
    if !webserver.check_privileged_access() return nil end
      ...
      if webserver.has_arg("action")
        ...
        if action == "ownership"
          ...
          if webserver.has_arg("mqtt_username") && webserver.has_arg("mqtt_password")
            ...
            var b = bytes(mqtt_password)
            var encoded = b.tob64()
            tasmota.cmd(string.format("Publish tele/349454CC01D4/mqqt/change_credentials {'username':'%s','password':'%s'}",mqtt_username,encoded),true)
          end
        end  
      end
